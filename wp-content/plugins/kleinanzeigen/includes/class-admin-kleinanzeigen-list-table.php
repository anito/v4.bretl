...
class Kleinanzeigen_List_Table extends WP_List_Table
{
  ...
  function __construct()
  {
    ...
    self::$INVISIBLE = __('Not visible', 'wbp-kleinanzeigen');
    self::$PRICE_DIFF = __('Price deviation', 'wbp-kleinanzeigen');
    self::$CONTAINS_DEFAULT_CAT = __('Contains Default Category', 'wbp-kleinanzeigen');
    self::$NO_CAT = __('Invalid Category', 'wbp-kleinanzeigen');
  }
  ...
  function render_head()
  {
    ...
    $featured_products = wbp_ka()->get_featured_products();

    $products = array('publish' => array(), 'draft' => array(), 'unknown' => array(), 'other' => array(), 'no-sku' => array(), 'todos' => array());
    foreach ($this->items as $item) {

      $product_by_sku = wbp_ka()->get_product_by_sku($item->id);
      if (!$product_by_sku) {
        $product_by_title = wbp_ka()->get_product_by_title($item->title);
      }
      ...
      $product ? (wbp_ka()->has_price_diff($item, $product) ? $products['todos'][] = array('title' => $item->title, 'id' => $item->id, 'reason' => self::$PRICE_DIFF) : null) : null;
    ...
    wbp_ka()->include_kleinanzeigen_template('header.php', false, compact('products', 'items', 'paged', 'categories', 'published_has_sku', 'published_no_sku', 'featured_products', 'tasks'));
  }
  ...
  function render_tasks()
  {
    ...
    foreach ($tasks as $key => $task) {
      $data = wbp_ka()->get_task_list_items($products, $ads, $task['name']);
  ...
  function render_row($record)
  {
    ...
    $product_by_sku = wbp_ka()->get_product_by_sku($record->id);
    if (!$product_by_sku) {
      $product_by_title = wbp_ka()->get_product_by_title($record->title);
    }
    ...
    if ($product) {
      ...
      if (wbp_ka()->has_price_diff($record, $product)) {
    ...
    <tr id="ad-id-<?php echo $record->id ?>" <?php if (!empty($diff_classes)) { ?>class="<?php echo implode(' ', $diff_classes) ?>" <?php } ?>>
      ...
      foreach ($columns as $column_name => $column_display_name) {
        ...
        if ($product) {
          ...
          if (!$product_by_sku) {
            $label = __('Connect', 'wbp-kleinanzeigen');
            $action = 'connect-' . $post_ID;
            $icon = 'admin-links';
            $type = 'button';
          } else {
            $label = __('Disconnect', 'wbp-kleinanzeigen');
          ...
          $kleinanzeigen_actions =
            '<div>' .
            wbp_ka()->include_kleinanzeigen_template('dashboard/kleinanzeigen-actions.php', true, array_merge(compact('post_ID', 'record', 'post_status', 'classes'), array('connected' => $product_by_sku))) .
            wbp_ka()->include_kleinanzeigen_template('dashboard/kleinanzeigen-toggle-link-control.php', true, compact('post_ID', 'record', 'classes', 'label', 'action', 'icon', 'type')) .
            '</div>';
        }
        ...
        if ($product_by_sku) {
          ...
          $shop_actions =
            '<div>' .
            wbp_ka()->include_kleinanzeigen_template('dashboard/common-links.php', true, compact('status_name', 'post_status', 'post_ID', 'record', 'classes', 'deletelink', 'editlink', 'permalink')) .
            wbp_ka()->include_kleinanzeigen_template('dashboard/toggle-publish-link.php', true, compact('post_status', 'post_ID', 'record')) .
            '</div>';
        } elseif ($product) {
          ...
          $icon = 'plus';
          $type = 'button';
          $kleinanzeigen_actions = wbp_ka()->include_kleinanzeigen_template('dashboard/kleinanzeigen-create-control.php', true, compact('record', 'action', 'icon', 'type'));
          $shop_actions = '';
        }
      ...
      ?>
          ...
          case "image": {
            ?>
              <td class="<?php echo $class ?>">
                <div class="column-content"><a href="<?php echo wbp_ka()->get_kleinanzeigen_url($record->url) ?>" target="_blank"><img src="<?php echo stripslashes($record->image) ?>" width="128" /></a></div>
              </td>
            <?php
              break;
            }
          ...
          case "title": {
            ?>
              <td class="<?php echo $class ?>">
                <div class="column-content"><a href="<?php echo wbp_ka()->get_kleinanzeigen_url($record->url) ?>" target="_blank"><?php echo $record->title ?></a></div>
              </td>
            <?php
              break;
            }
