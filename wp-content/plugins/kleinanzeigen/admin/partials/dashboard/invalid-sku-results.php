<div id="table-task-list-outer" class="list-outer">
  <div class="ka-list-header" style="display: flex; flex-direction: column;">
    <div style="display: flex; justify-content: center;">
      ...
      <h5><?php echo __('Evaluation Results', 'kleinanzeigen') ?></h5>
    </div>
    <div style="display: flex; justify-content: center;">
      <p style="margin-top: -20px; margin-bottom: 20px;"><?php echo __('Products based on not accessible ads', 'kleinanzeigen') ?></p>
    </div>
  </div>
        ...
        <?php if (empty($data['products'])) : ?>
          <tr>
            <td colspan="3" class="" style="height: 60px; text-align: center;">
              <?php echo __('No products found', 'kleinanzeigen') ?>
            </td>
          </tr>
        <?php else : ?>
          <?php foreach ($data['products'] as $published) : ?>
            <tr id="<?php echo $published['post_ID'] ?>">
              ...
              <td class="column-actions">
                <a href="#" type="button" class="button button-primary button-small action-button deactivate" data-action="deactivate" data-success-label="<?php echo __('Deactivated', 'kleinanzeigen') ?>" data-post-id="<?php echo $published['post_ID'] ?>" data-kleinanzeigen-id="<?php echo $published['sku'] ?>" data-disconnect="disconnect" data-screen="modal"><?php echo __('Deactivate', 'kleinanzeigen') ?></a>
                <a href="#" type="button" class="button button-primary button-small action-button disconnect" data-action="disconnect" data-success-label="<?php echo __('Disconnected', 'kleinanzeigen') ?>" data-post-id="<?php echo $published['post_ID'] ?>" data-kleinanzeigen-id="<?php echo $published['sku'] ?>" data-screen="modal"><?php echo __('Just disconnect', 'kleinanzeigen') ?></a>
              </td>
            </tr>
          <?php endforeach ?>
        <?php endif ?>
...
<div class="button-controls left">
  <div class="button-group">
    <a href="#" type="button" class="button action-button button-primary deactivate-all <?php echo empty($data['products']) ? 'disabled' : '' ?>" data-action="deactivate-all"><?php echo __('Deactivate all', 'kleinanzeigen') ?></a>
    <a href="#" type="button" class="button action-button button-primary disconnect-all <?php echo empty($data['products']) ? 'disabled' : '' ?>" data-action="disconnect-all"><?php echo __('Just disconnect all', 'kleinanzeigen') ?></a>
  </div>
</div>
