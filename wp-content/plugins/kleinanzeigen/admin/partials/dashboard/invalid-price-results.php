<div id="table-task-list-outer" class="list-outer">
  <div class="ka-list-header" style="display: flex; flex-direction: column;">
    ...
    <div style="display: flex; justify-content: center;">
      <p style="margin-top: -20px; margin-bottom: 20px;"><?php echo __('Products with price differences', 'kleinanzeigen') ?></p>
    </div>
  </div>
            ...
            <tr id="<?php echo $published['post_ID'] ?>">
              ...
              <td class="column-shop-price">
                <div class="ad-price price">
                  <span><?php echo __('KA price', 'kleinanzeigen') ?>:</span>
                  <span><?php echo $published['ka_price'] ?></span>
                </div>
                <div class="shop-price price">
                  <span><?php echo __('Shop price', 'kleinanzeigen') ?>:</span>
                  <span><?php echo $published['shop_price'] ?></span>
                </div>
              </td>
              <td class="column-actions">
                <a href="#" type="button" class="button button-primary button-small action-button fix-price" data-action="fix-price" data-success-label="<?php echo __('Fixed', 'kleinanzeigen') ?>" data-post-id="<?php echo $published['post_ID'] ?>" data-kleinanzeigen-id="<?php echo $published['sku'] ?>" data-price="<?php echo $published['price'] ?>" data-ka-price="<?php echo $published['ka_price'] ?>" data-screen="modal"><?php echo __('Fix price', 'kleinanzeigen') ?></a>
              </td>
            </tr>
          <?php endforeach ?>
        <?php endif ?>
...
<div class="button-controls left">
  <a href="#" type="button" class="button action-button button-primary fix-price-all <?php echo empty($data['products']) ? 'disabled' : '' ?>" data-action="fix-price-all"><?php echo __('Fix all prices', 'kleinanzeigen') ?></a>
</div>
